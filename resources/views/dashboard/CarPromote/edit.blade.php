...
@section('content')
    ...
    <div class="card card-custom">
        ...
        <!--begin::Form-->
        <form action="{{route("dashboard.promote.update",['promote'=>$promote->id])}}" method="POST">
            @csrf
            @method('PATCH')
            <div class="card-body">
                <!-- EN Form -->
                        ...
                        <div class="form-group row">
                            ...
                            <div class=" col-lg-9 col-md-9 col-sm-12">
                             <select class="form-control select2 {{ $errors->has('car_id') ? 'is-invalid' : '' }}"
                                ...
                                @foreach ($cars as $car)
                                    <option value="{{$car->id}}" {{ $promote->car_id==$car->id ? 'selected':'' }} >
                            ...
                            <div class=" col-lg-9 col-md-9 col-sm-12">
                             <select class="form-control select2 {{ $errors->has('subscribe_package_id') ? 'is-invalid' : '' }}"
                                ...
                                @foreach ($packages as $package)
                                    <option value="{{$package->id}}"{{ $promote->subscribe_package_id==$package->id ? 'selected':'' }} >
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}"
                             name="price" value="{{ old('price') ?? $promote->price}}"required placeholder="@lang('price')"/>
                            @error('price')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control {{ $errors->has('weaccept_order_id') ? 'is-invalid' : '' }}"
                             name="weaccept_order_id" value="{{ old('weaccept_order_id') ?? $promote->weaccept_order_id }}"required placeholder="@lang('Order ID')"/>
